...
/* current state */
action_t current_state = RANDOM_WALKING;
...
void rx_message(message_t *msg, distance_measurement_t *d) {
    ...
    if(new_sa_msg==true){      //light up LED in desired color and eventually change state
            set_color(RGB(sa_payload/100%10,sa_payload/10%10,sa_payload%10));
            if((sa_type==2)&&(current_state==RANDOM_WALKING)){
                current_state=SEARCHING;
                set_motion(FORWARD);
                last_motion_ticks=kilo_ticks;
            }
            if((sa_type==1)&&(current_state==SEARCHING)){
                current_state=INSIDE_AREA;
                set_motors(0,0);
                set_motion(STOP);
            }
            if((sa_type==2)&&(current_state==INSIDE_AREA)){
                current_state=RANDOM_WALKING;
                set_motion(FORWARD);
            }
            if((sa_type==0)&&(current_state==INSIDE_AREA)){
                current_state=RANDOM_WALKING;
                set_motion(FORWARD);
                last_motion_ticks=kilo_ticks;
            }
