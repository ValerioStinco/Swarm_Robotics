...
if [ "$#" -ne 2 ]; then
    echo "Usage: simple_experiment_dhtf.sh (from src folder) <base_config_dir> <base_config_file_name>"
    exit 11
fi
...
for _K_ in $entity_quantity; do
    for _T_ in $timeout_const; do
        ...
        param_dir=$res_dir/$date_time"_robots#"$_K_"_timeout_const#"$_T_"_augmmented_knowledge#"$_A_"_"$experiment_length
        if [[ ! -e $param_dir ]]; then
            mkdir $param_dir
            ...
            for it in $(seq 1 $RUNS); do
                ...
                sed -i "s|__ENTITY_QUANTITY__|$_K_|g" $config
                sed -i "s|__TIMEOUT_CONST__|$_T_|g" $config
                sed -i "s|__AUGMENTED_KNOWLEDGE__|$_A_|g" $config                
                sed -i "s|__EXPERIMENT_LENGTH__|$experiment_length|g" $config
