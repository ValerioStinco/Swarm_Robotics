...
/*Update kilobot state*/
        ...
        if (m_vecKilobotStates[unKilobotID]==SEARCHING){    //When in SEARCHING MODE kilobots will stop as soon as they reach target area and the number of arrived unities is send via socket
            ...
            if(fDistance<(multiArea[target_index].Radius*1)){
                ...
                if (inPlace[unKilobotID]==0){
                    ...
                    std::cout<<"kilobots on target: "<<buf<<std::endl;
    ...
    if(MODE=="CLIENT"){
        ...
        for(int i=0;i<num_of_areas;i++){            
            ...
            if((fDistance<(multiArea[i].Radius*1)) && (multiArea[i].Free == 0)){
                m_vecKilobotStates[unKilobotID]=INSIDE_AREA;
    ...
    if (m_fTimeInSeconds - m_vecLastTimeMessaged[unKilobotID]< m_fMinTimeBetweenTwoMsg){
        ...
        if(MODE=="SERVER"){
            if(bufStore[0]==51 && bufStore[1]==51 && bufStore[2]==51){ //333 in ASCII, this detects the starting message and lights up white led
                m_vecKilobotData[unKilobotID].ALL=atoi(bufStore);       //use this when receiving RGB code to light up in that color
            }
...
REGISTER_LOOP_FUNCTIONS(CALFClientServer, "kilobot_ALF_client_loop_function")